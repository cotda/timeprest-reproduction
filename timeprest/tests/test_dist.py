...
from timeprest.reference import full_grad_at, mixed_rule_grads
...
def engine_reference(system, model, K, N, epochs=1):
    pc = pipeline_cfg(system, N, "static", "recompute", 2)
...
@pytest.mark.parametrize("system,model,mode", [("timeprest", "mlp", "recompute"), ("timeprest", "mlp", "auto"),
                                               ("pipedream", "mlp", "auto"), ("timeprest", "vgg", "auto"),
                                               ("pipedream", "vgg", "auto")])
def test_static_order_matches_single_process_engine(tmp_path, system, model, mode):
    """Static (Fig.2) order on 2 processes == phase-1 engine: same versions, same parameters."""
    res = run_dist(tmp_path, system=system, model=model, order="static", backward_mode=mode, K=6, N=3, epochs=2)
    eng = engine_reference(system, model, 6, 3, epochs=2)
    for s in range(2):
...
def test_dynamic_order_gradients_follow_rule(tmp_path, system):
    ...
    K, N = 8, 3
    res = run_dist(tmp_path, system=system, model="mlp", order="dynamic", backward_mode="auto", K=K, N=N)
    ...
    for i, (x, y) in enumerate(batches):
        if system == "timeprest":
            ...
            ref = mixed_rule_grads(stages, vp, [fwd[0][(i, j)] for j in range(N)], kb, x, y, N)
        else:
